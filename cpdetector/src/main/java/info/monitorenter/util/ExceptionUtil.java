...
public final class ExceptionUtil {
	 ...
	 * Application to {@link System#err}.
	 * <p>
	 * <b>Caution</b><br/>
	 * If you do not consume the bytes to read from the result you may block the
	 * whole application. Do only use this for debugging purposes or end to end
	 * test code!
	 * <p>
	 * Attempting to read from the result in the same thread that called here is
	 * not recommended as it may deadlock the thread. Also the thread reading
	 * from the stream result should not write anything to {@link System#err}.
	 * <p>
	 * Prefer using {@link #findMatchInSystemErr(String)} to avoid deadlocks.
	 * <p>
	 * 
	 * @param teeToOriginalSysErr
	 *            if true, all writes to the newly installed {@link System#err}
	 *            will also sent to the original one (thus be visible on the
	 *            console if the original one was untouched before).
	 * @return an input stream that contains what will written in this
	 *         Application to {@link System#err}.
	 * @throws IOException
	...
	/**
	 * Returns an input stream that contains what will written in this
	 * Application to {@link System#out}.
	 * <p>
	 * <b>Caution</b><br/>
	 * If you do not consume the bytes to read from the result you may block the
	 * whole application. Do only use this for debugging purposes or end to end
	 ...
	 * Attempting to read from the result in the same thread that called here is
	 * not recommended as it may deadlock the thread. Also the thread reading
	 * from the stream result should not write anything to {@link System#out}.
	 * <p>
	 * Prefer using {@link #findMatchInSystemOut(String)} to avoid deadlocks.
	 * <p>
	 * 
	 * @param teeToOriginalSysOut
	 *            if true, all writes to the newly installed {@link System#out}
	 *            will also sent to the original one (thus be visible on the
	 *            console if the original one was untouched before).
	 * @return an input stream that contains what will written in this
	 *         Application to {@link System#out}.
	...
	/**
	 * Returns an instance to a runnable running in a separate Thread that tries
	 ...
	 * {@link #captureSystemOutForDebuggingPurposesOnly(boolean)} as this will
	 * avoid blocking your application.
	 * <p>
	 * 
	 * @return an instance to a runnable running in a separate Thread that tries
	 *         to match the expected output in {@link System#out}.
	 * @throws IOException
	 *             if something goes wrong.
	 */
	 ...
	 * <p>
	 * Prefer this instead of
	 * {@link #captureSystemErrForDebuggingPurposesOnly(boolean)} as this will
	 * avoid blocking your application.
	 * <p>
	 * 
	 * @return an instance to a runnable running in a separate Thread that tries
	...
	public static class InputStreamTracer implements Runnable {
		 ...
		 * <p>
		 * Note that it may be time - critical when to call this and take for
		 * granted that the match was not made in the input stream
		 * (concurrency).
		 * <p>
		 * 
