...
public class CrudRepositoryImpl<T, ID extends Serializable> implements CrudRepository<T, ID> {
    ...
    @Override
    public <RT> List<RT> search(CriterionQuery parameters) {
        if (parameters == null) {
    ...
    @Override
    public <RT> RT searchUnique(CriterionQuery parameters) {
        return (RT) repository._searchUniqueEntity(persistent,
                parameters);
    }

    @Override
    public int count(CriterionQuery parameters) {
        if (parameters == null) {
            parameters = (CriterionQuery) new Search();
        }
        return repository._countEntity(persistent, parameters);
    }

    @Override
    public <RT> SearchResult<RT> searchAndCount(CriterionQuery parameters) {
        if (parameters == null) {
