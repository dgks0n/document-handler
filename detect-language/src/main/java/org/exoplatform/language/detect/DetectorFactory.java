...
public class DetectorFactory {
   ...
   *           Can't open profiles(error code = {@link ErrorCode#FileLoadError})
   *           or profile's format inputStream wrong (error code =
  ...
  public static void loadLanguageProfiles(String... profiles)
    ...
    if (logger.isDebugEnabled()) {
      logger
          .info("Load profiles from specified directory \"" + profiles + "\"");
    }

    for (int i = 0; i < profiles.length; i++) {
      InputStream inputStream = LanguageProfile.class
          .getClassLoader()
      ...
      try {
        ...
        throw new LanguageDetectException("Profile format error in for: "
            + profiles[i], ErrorCode.FORMATERROR);
      } catch (IOException e) {
        throw new LanguageDetectException("Profile format error in for: "
            + profiles[i], ErrorCode.FORMATERROR);
      } finally {
   ...
   * @throws LanguageDetectException
   */
  protected static void addLanguageProfile(LanguageProfile profile, int langsize)
      throws LanguageDetectException {
    if (logger.isDebugEnabled()) {
    ...
    if (_instance.languages.contains(language)) {
      throw new LanguageDetectException("Duplicate the same language profile",
   ...
   * @throws LanguageDetectException
   */
  public static Detector createInstance(double alpha)
      throws LanguageDetectException {
  ...
  private static Detector createDetector() throws LanguageDetectException {
    if (_instance.languages.size() == 0) {
      throw new LanguageDetectException("Need to load profiles",
          ErrorCode.NEEDLOADPROFILEERROR);
    }
