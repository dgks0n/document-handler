...
public class HibernateTransactionManager implements ITransactionManager {
  ...
  public HibernateTransactionManager(HibernateConfiguration configuration) {
    ...
    this.metadataUtil = HibernateMetadataUtil.getInstanceForSessionFactory(configuration.getSessionFactory());
    this.searchProcessor = HibernateSearchProcessor.getInstanceForSessionFactory(configuration.getSessionFactory());
  }
   ...
   * guarantee that the object will be assigned an identifier immediately. With
  ...
  /**
   * Persist the given transient instances and add them to the datastore, first
   * assigning a generated identifier. (Or using the current value of the
   * identifier property if the assigned generator is used.) This operation
   * cascades to associated instances if the association is mapped with
   * cascade="save-update".
   ...
   * 
   * <p>
   * Either <code>save()</code> or <code>update()</code> based on the following
   * rules
   * <ul>
   ...
   * <li>if the object is versioned (by a &lt;version&gt; or &lt;timestamp&gt;),
  ...
  /**
   * <p>
   * If an entity already exists in the datastore with the same id, call _update
   * and return false (not new). If no such entity exists in the datastore, call
   * _save() and return true (new)
   * 
  ...
  /**
   * Either <code>save()</code> or <code>update()</code> each entity, depending
   * on whether or not an entity with the same id already exists in the
   * datastore.
   * 
  ...
  /**
   * <p>
   * Return the all the persistent instances of the given entity class with the
   * given identifiers. An array of entities is returned that matches the same
   ...
   * uninitialized proxy and does not actually hit the database until you invoke
   * a method of the proxy. This behaviour is very useful if you wish to create
   * an association to an object without actually loading it from the database.
   ...
   * each entity that is not found in the datastore, a null will be inserted in
   ...
   * persistent instance. The given instance does not become associated with the
   * session. This operation cascades to associated instances if the association
   * is mapped with cascade="merge".
   * 
   * <p>
   * The difference between <code>update()</code> and <code>merge()</code> is
   * significant: <code>update()</code> will make the given object persistent
  ...
  /**
   * Returns the total number of results that would be returned using the given
   * <code>SearchParameters</code> if there were no paging or maxResult limits.
   * 
   * @see SearchCriterion
   */
  ...
  /**
   * Same as <code>_countEntity(SearchParameters)</code> except that it uses the
   * specified search class instead of getting it from the search object. Also,
  ...
  /**
   * Same as <code>_searchAndCountEntity(SearchParameters)</code> except that it
